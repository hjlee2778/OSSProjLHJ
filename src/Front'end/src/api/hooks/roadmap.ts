...
export const usePostDuelApply = () => {
  const { mutateAsync, isLoading } = useMutation(postDuelApply);

  const postDuelApplyAsync = async (req: { roadmapId: number }) => {
    const { roadmapId } = req;
    if (roadmapId > 0) {
      const data = await mutateAsync(req);
  ...
  return { postDuelApplyAsync, isLoading };
};
...
export const useGetRoadmapsMyList = () => {
  ...
  return { groups: data?.result.roadmaps.groups, duels: data?.result.roadmaps.duels, isLoading };
};

...
function handleMessage(request, sender, sendResponse) {
  if (request && request.closeWebPage === true && request.isSuccess === true) {
    /* Set username */
    chrome.storage.local.set({ DUELHub_username: request.username });

    /* Set token */
    chrome.storage.local.set({ DUELHub_token: request.token });

    /* Close pipe */
    chrome.storage.local.set({ pipe_DUELHub: false });

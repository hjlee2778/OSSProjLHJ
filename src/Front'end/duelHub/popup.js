        ...
        if (xhr.status === 200) {
          ...
          chrome.storage.local.get('mode_type', (data2) => {
            if (data2 && data2.mode_type === 'commit') {
              ...
              chrome.storage.local.get(['DUELHub_hook'], (data3) => {
                const BaekjoonHubHook = data3.DUELHub_hook;
                if (BaekjoonHubHook) {
                  $('#repo_url').html(
          ...
          chrome.storage.local.set({ DUELHub_token: null }, () => {
 ...
 */
chrome.storage.local.get('DUELEnable', (data4) => {
  if (data4.DUELEnable === undefined) {
    $('#onffbox').prop('checked', true);
    chrome.storage.local.set({ DUELEnable: $('#onffbox').is(':checked') }, () => {});
  } else {
    $('#onffbox').prop('checked', data4.DUELEnable);
    chrome.storage.local.set({ DUELEnable: $('#onffbox').is(':checked') }, () => {});
  }
});
/*

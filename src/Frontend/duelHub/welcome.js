...
const statusCode = (res, status, name) => {
  switch (status) {
    ...
    default:
      ...
      /* Set Repo Hook */
      chrome.storage.local.set({ DUELHub_hook: res.full_name }, () => {
...
const createRepo = (token, name) => {
  ...
  let data = {
    ...
    auto_init: true,
    description:
      'A space to record and share every step of a steady developer [DUEL](https://github.com/CSID-DGU/2024-1-OSSProj-Questionmark-07).',
  };
    ...
    if (xhr.readyState === 4) {
      ...
      if (xhr.status === 200) {
        ...
        if (!bool) {
          ...
          chrome.storage.local.set({ DUELHub_hook: null }, () => {
...
$('#hook_button').on('click', () => {
  ...
  if (!option()) {
    ...
    */
    chrome.storage.local.get('DUELHub_token', (data) => {
      const token = data.DUELHub_token;
      if (token === null || token === undefined) {
        ...
        chrome.storage.local.get('DUELHub_username', (data2) => {
          const username = data2.DUELHub_username;
          if (!username) {
...
chrome.storage.local.get('mode_type', (data) => {
  ...
  if (mode && mode === 'commit') {
    ...
    chrome.storage.local.get('DUELHub_token', (data2) => {
      const token = data2.DUELHub_token;
      ...
      if (token === null || token === undefined) {
        ...
        chrome.storage.local.get('DUELHub_hook', (repoName) => {
          const hook = repoName.DUELHub_hook;
          if (!hook) {
            /* Not authorized yet. */
            $('#error').text(

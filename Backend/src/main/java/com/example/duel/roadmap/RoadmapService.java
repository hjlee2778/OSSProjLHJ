...
public class RoadmapService {
    ...
    // 듀얼 로드맵 생성하기 - 임시 api
    @Transactional
    public RoadmapResponse.CreateRoadmapDTO createDuelRoadmap(RoadmapRequest.CreateDuelRoadmapDTO requestDTO, User user){
        Roadmap roadmap = Roadmap.builder()
                .creator(user)
                .category(Category.CATEGORY_DUEL)
    ...
    public RoadmapResponse.FindAllMyRoadmapDTO findAllMyRoadmaps(User user) {
        ...
        List<RoadmapResponse.DuelDTO> duels = roadmaps.stream()
                .filter(roadmap -> roadmap.getCategory().equals(Category.CATEGORY_DUEL))
                .map(RoadmapResponse.DuelDTO::new).collect(Collectors.toList());
        ...
        return new RoadmapResponse.FindAllMyRoadmapDTO(categories, new RoadmapResponse.FindAllMyRoadmapDTO.RoadmapDTO(duels, groups));
    }
    ...
    // duel 로드맵에 참여 신청하기
    @Transactional
    public void applyDuelRoadmap(Long roadmapId, User user) {
        Roadmap roadmap = getRoadmapById(roadmapId);
